...
class SDNE(StaticGraphEmbedding):
    ...
    def learn_embedding(self, graph=None, edge_f=None,
        ...
        # Outputs
        x_diff1 = subtract([x_hat1, x1])#,mode=lambda ab: ab[0] - ab[1],output_shape=lambda L: L[1])
        x_diff2 = subtract([x_hat2, x2])#,mode=lambda ab: ab[0] - ab[1],output_shape=lambda L: L[1])
        y_diff = subtract([y2, y1])#,mode=lambda ab: ab[0] - ab[1],output_shape=lambda L: L[1])
        ...
        #la je sais pas trop trop ce que je fais
        #self._model.fit_generator(
        #    generator=batch_generator_sdne(S, self._beta, self._n_batch, True),
        #    nb_epoch=self._num_iter,
        #    samples_per_epoch=S.nonzero()[0].shape[0] // self._n_batch,verbose=1)

        self._model.fit_generator(
            generator=batch_generator_sdne(S, self._beta, self._n_batch, True),
            epochs=self._num_iter,
            #samples_per_epoch=S.nonzero()[0].shape[0] // self._n_batch,verbose=1)
            steps_per_epoch=S.nonzero()[0].shape[0] // self._n_batch,verbose=1)
